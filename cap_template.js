...
function getTypeIcon(aType) {
	...
	return '<a href="/dex/sm/types/' + lowerType +
...
function topHeaderInfo(cap) {
	...
	if (cap.customAbility1 == null) {
		txt += '<dt> \n';
		txt += '<a href="/dex/sm/abilities/' + lowerAndJoin(cap.ability1) + '">' + cap.ability1 + '</a> \n';
		txt += '</dt> \n';
	...
	if (cap.customAbility2 == null) {
		if (cap.ability2 !== '') {
			txt += '<dt> \n';
			txt += '<a href="/dex/sm/abilities/' + lowerAndJoin(cap.ability2) + '">' + cap.ability2 + '</a> \n';
			txt += '</dt> \n';
	...
	if (cap.ability3 !== '') {
		txt += '<dt> \n';
		txt += '<em><a href="/dex/sm/abilities/' + lowerAndJoin(cap.ability3) + '">' + cap.ability3 + '</a></em> \n';
		txt += '</dt> \n';
...
function composeDexPage(cap) {
	...
	if (cap.topicLeadershipTeam !== '') {
		txt += '<dt>Topic Leadership Team</dt> \n';
		txt += '<dd>' + cap.topicLeadershipTeam + '</dd> \n';
	}
	if (cap.conceptCreator !== '') {
		txt += '<dt>Concept</dt> \n';
		txt += '<dd>' + cap.conceptCreator + '</dd> \n';
	}
	if (cap.statSpreadCreator !== '') {
...
function composeSet(set) {
	...
	txt += '<td> \n';
	txt += slashAndLink('/dex/sm/items/', set.item) + ' \n';
	txt += '</td> \n';
	txt += '<td> \n';
	txt += slashAndLink('/dex/sm/abilities/', set.ability) + ' \n';
	txt += '</td> \n';
	txt += '<td> \n';
	txt += set.nature + ' \n';
	txt += '</td> \n';
	...
	for (var m = 0; m < set.move.length; m++) {
		if (set.move[m] !== null) {
			txt += '~ ' + slashAndLink('/dex/sm/moves/', set.move[m]);
			txt += '<br /> \n';
		}
	}
	txt += '</td> \n';
	...
	for (var i = 1; i < caps.length; i++) {
		...
		if (currentGen != capGen) {
			...
			if (capGen == 'DP') {
				...
				txt += '<h2>Generation 7 - Sun/Moon</h2>';
			}
			txt += '</td> \n';
			txt += '</tr> \n';
			currentGen = capGen;
...
function composeCustomMovePage() {
	...
	if (urlParts[1] == "paleo_wave") {
		move = PaleoWave();
	} else if (urlParts[1] == "shadow_strike") {
		move = ShadowStrike();
	}
	...
	txt += '</tr> \n';
	txt += '<tr> \n';
	txt += '<td><a href="/dex/sm/types/' + move.type.toLowerCase() + '">' + move.type + '</a></td> \n';
	txt += '<td>' + move.power + '</td> \n';
	txt += '<td>' + move.accuracy + '</td> \n';
	txt += '<td>' + move.pp + '</td> \n';
